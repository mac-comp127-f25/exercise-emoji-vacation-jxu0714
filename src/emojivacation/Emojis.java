...
public class Emojis {
    ...
    public static GraphicsGroup createSmileyFace(double size) {
        ...
        // TODO: create eyes
        Ellipse eyes = createEyes(size * 0.1, size * 0.1);
        eyes.setCenter(size * 0.325, size * 0.4);
        group.add(eyes);

        Ellipse eyes2 = createEyes(size * 0.1, size * 0.1);
        eyes2.setCenter(size * 0.65, size * 0.4);
        group.add(eyes2);
    ...
    private static Ellipse createEyes(double height, double width) {
        Ellipse eyes = new Ellipse(0, 0, width, height);
        eyes.setFillColor(EYES_COLOR);
        // eyes.setStrokeColor(EYES_COLOR);
        return eyes;
    }
